                      ...
                      >
                        <div className="flex items-start gap-3.5">
                          ...
                          <button
                            ...
                            className="mt-1 transition-transform active:scale-90 shrink-0"
                          >
                            {task.completed ? (
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {/* Description */}
                            {task.description && (
                              <p className="text-sm text-slate-500 dark:text-slate-400 mb-3 break-words">
                                {task.description}
                              </p>
                            )}
                            ...
                            {totalSubtasks > 0 && (
                              <div className="my-3 pl-3 border-l-2 border-slate-200 dark:border-slate-800 space-y-1.5">
                                <div className="text-xs font-semibold text-slate-400 flex items-center gap-2 mb-1">
                                  <ListPlus className="w-3.5 h-3.5" />
                                  <span>Công việc con ({completedSubtaskCount}/{totalSubtasks})</span>
                                </div>
                                {task.subtasks.map(sub => (
                                  <div key={sub.id} className="flex items-center gap-2 text-xs text-slate-600 dark:text-slate-300">
                                    <button 
                            ...
                            {/* Date, Time & Reminder Info */}
                            <div className="flex flex-wrap items-center gap-4 text-xs text-slate-400 dark:text-slate-500 mt-2">
                              <div className={`flex items-center gap-1 font-medium ${
                                isOverdue ? 'text-red-500' : isToday ? 'text-blue-500' : ''
                              }`}>
                                <Calendar className="w-3.5 h-3.5" />
                                <span>{task.dueDate} {task.dueTime}</span>
                                {isOverdue && <span className="font-bold">(Quá hạn)</span>}
                                {isToday && !task.completed && <span className="font-bold">(Hôm nay)</span>}
                              </div>

                              {task.reminderOffset !== undefined && task.reminderOffset >= 0 && (
                                <div className="flex items-center gap-1">
                                  <BellRing className="w-3.5 h-3.5 text-amber-500" />
                                  <span>
                                    Nhắc trước {REMINDER_OPTIONS.find(r => r.value === task.reminderOffset)?.label || `${task.reminderOffset} phút`}
                                  </span>
                                </div>
                              )}
                            </div>
                          ...
                          <div className="flex items-center gap-1 opacity-80 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <button
                              ...
                              className="p-1.5 hover:bg-rose-50 dark:hover:bg-rose-950/40 rounded-lg text-slate-400 hover:text-rose-500 transition-colors"
                              title="Xóa công việc"
                            >
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                ...
                <p className="text-3xl font-bold mt-2 text-slate-800 dark:text-slate-100">{stats.total}</p>
                <p className="text-xs text-slate-400 mt-1">Đã khởi tạo trong hệ thống</p>
              </div>

              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <p className="text-xs font-semibold text-emerald-500 uppercase">Tỷ lệ hoàn thành</p>
                <p className="text-3xl font-bold mt-2 text-emerald-600 dark:text-emerald-400">{stats.completionRate}%</p>
                <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2 mt-3 overflow-hidden">
                  <div className="bg-emerald-500 h-2 rounded-full transition-all duration-500" style={{ width: `${stats.completionRate}%` }}></div>
                </div>
              </div>

              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <p className="text-xs font-semibold text-blue-500 uppercase">Đang chờ thực hiện</p>
                <p className="text-3xl font-bold mt-2 text-blue-600 dark:text-blue-400">{stats.pending}</p>
                <p className="text-xs text-slate-400 mt-1">Cần hoàn thành sắp tới</p>
              </div>

              <div className={`p-5 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
                <p className="text-xs font-semibold text-rose-500 uppercase">Công việc quá hạn</p>
                <p className="text-3xl font-bold mt-2 text-rose-600 dark:text-rose-400">{stats.overdue}</p>
                <p className="text-xs text-slate-400 mt-1">Cần xử lý ngay lập tức</p>
              </div>
            </div>

            {/* Category Distribution Breakdown */}
            <div className={`p-6 rounded-2xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'}`}>
              <h3 className="text-base font-bold text-slate-800 dark:text-slate-100 mb-4">Phân bố công việc theo danh mục</h3>
                    ...
                    <div key={cat.id} className="space-y-1.5">
                      <div className="flex justify-between text-sm font-medium">
                        ...
                        <span className="text-slate-500">
                          {catCompleted}/{catTotal} việc ({percent}%)
                        </span>
                      </div>
                      <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2.5 overflow-hidden">
                        <div className={`${cat.color} h-2.5 rounded-full transition-all duration-500`} style={{ width: `${percent}%` }}></div>
                      </div>
                    </div>
                  );
                })}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fadeIn">
          <div className={`w-full max-w-lg rounded-2xl p-6 shadow-2xl border max-h-[90vh] overflow-y-auto ${
            ...
            <form onSubmit={handleSaveTask} className="space-y-4">
              ...
              {/* Subtasks Builder */}
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1">Công việc con (Subtasks)</label>
                <div className="flex gap-2 mb-2">
                  <input
                    type="text"
                    placeholder="Thêm các bước nhỏ..."
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-slate-200 dark:bg-slate-800 rounded-xl text-xs font-semibold hover:bg-slate-300"
                  >
                    Thêm
                  </button>
                </div>
                {formSubtasks.length > 0 && (
                  <div className="space-y-1.5 max-h-32 overflow-y-auto">
                    {formSubtasks.map((st) => (
                      <div key={st.id} className="flex items-center justify-between text-xs p-2 rounded-lg bg-slate-100 dark:bg-slate-800/50">
                        <span>{st.title}</span>
                        <button type="button" onClick={() => removeSubtaskFromForm(st.id)} className="text-red-400 hover:text-red-600">
                          <X className="w-3.5 h-3.5" />
                        </button>
                      </div>
                    ))}
              ...
              {/* Reminder Selector */}
              <div>
                <label className="block text-xs font-bold uppercase text-slate-400 mb-1">Thông báo nhắc nhở</label>
                <select
                  value={formReminder}
              ...
              <div className="flex items-center justify-end gap-2 pt-4 border-t border-slate-200 dark:border-slate-800">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-xl text-sm font-medium bg-blue-600 hover:bg-blue-700 text-white shadow-md shadow-blue-500/20"
                >
